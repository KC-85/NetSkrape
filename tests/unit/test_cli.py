...
def test_crawl_command_reports_database_destination(
    monkeypatch,
    tmp_path: Path,
) -> None:
    """The CLI identifies SQLite as the selected persistence backend."""
    ...
    result = CliRunner().invoke(
        cli.main,
        ["crawl", "https://example.com", "--database", str(database)],
    )

    assert result.exit_code == cli.SUCCESS
    assert f"SQLite database {database}" in result.output
